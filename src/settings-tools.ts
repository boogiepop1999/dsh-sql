...
function isConnectionTable(value: unknown): value is Record<string, SqlConnectionConfig> {
...
export function buildSettingsTools(): SqlToolDefinition[] {
  ...
  const sqlConfigSet: SqlToolDefinition = {
    ...
    async execute(rawArgs) {
      ...
      // 环境清单：必须是非空字符串数组，去重。
      let nextEnvironments: string[] | undefined
  ...
  const sqlConnectionSet: SqlToolDefinition = {
    ...
    description:
      '新增或覆盖一个连接。**未给的字段保持不变；给了就设为该值（空串即空串）**。先 sql_settings 看现值。立即生效，不用重启。\n' +
      CONFIG_WRITE_WARNING,
    parameters: compileParameters({
      ...
      env: { type: 'string', description: '所属环境（如 qa / prod）；留空表示不限定环境（任何环境都可用）。' },
    ...
    async execute(rawArgs) {
      ...
      if (typeof engineRaw === 'string' && engineRaw.trim() !== '') {
        engine = engineRaw.trim() as SqlConnectionConfig['engine']
      ...
      const base: SqlConnectionConfig = existing !== undefined
        ? JSON.parse(JSON.stringify(existing)) as SqlConnectionConfig
        : { engine }
       ...
       */
      const applyText = (key: 'file' | 'host' | 'user' | 'password' | 'database' | 'env' | 'description'): void => {
      ...
      const applyPort = (): void => {
        ...
        if (!Number.isInteger(port) || port <= 0) throw new Error('port 必须是正整数，收到 ' + JSON.stringify(value) + '。')
        base.port = port
      }
      ...
      if (entry.env !== undefined && entry.env !== '') {
        ...
        if (!known.includes(entry.env)) {
          ...
          throw new Error(
            ...
            '先用 sql_config_set 把它加进 environments，或留空表示不限定环境。',
          )
        }
      }
