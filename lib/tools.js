...
import { isReadOnly, requireMaxRows, passwordEnvName, splitConnectionsByEnv, QUERY_TIMEOUT_MS, EXEC_TIMEOUT_MS, STATS_TIMEOUT_MS } from './config.js';
...
async function databaseSize(adapter, engine, database, signal) {
    if (engine === 'sqlite') {
    ...
    if (engine === 'mysql') {
        ...
        const result = await adapter.query("SELECT SUM(DATA_LENGTH + INDEX_LENGTH) FROM information_schema.tables WHERE TABLE_SCHEMA = '" + String(database).replace(/'/g, "''") + "'", 1, signal);
    ...
    const sqlStats = {
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (typeof rec.tablesError === 'string' && rec.tablesError !== '')
                    lines.push('⚠ 表清单不可用：' + rec.tablesError);
                if (typeof rec.sizeError === 'string' && rec.sizeError !== '')
                    lines.push('⚠ 库体积不可用：' + rec.sizeError);
