...
function describeDriverError(error) {
    if (error instanceof AggregateError && error.errors.length > 0) {
        ...
        for (const item of error.errors) {
            ...
            if (isBlankDetail(converted))
                blank += 1;
            else
                parts.push(converted.message);
        }
        if (blank > 0)
            parts.push(String(blank) + ' 个子错误未提供信息');
...
function blankDetail(name) {
    const error = new Error('未知错误（' + name + ' 未给出任何信息）');
    error.blankDetail = true;
    return error;
}
function isBlankDetail(error) {
    return error.blankDetail === true;
}
