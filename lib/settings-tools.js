 ...
 */
import { DESCRIPTION_MAX_LENGTH, EXEC_TIMEOUT_MS, QUERY_TIMEOUT_MS, STATS_TIMEOUT_MS, missingConnectionFields, resolveSettings, splitConnectionsByEnv, } from './config.js';
...
export function buildSettingsTools() {
    ...
    const sqlConfigSet = {
        name: 'sql_config_set',
        description: '改全局设置：activeEnv / environments / maxRows，至少一个入参。先 sql_settings 看现值。立即生效，不用重启。\n' +
    ...
    const sqlConnectionSet = {
        ...
        description: '新增或覆盖一个连接。**未给的字段保持不变；给了就设为该值**。先 sql_settings 看现值。立即生效，不用重启。\n' +
            CONFIG_WRITE_WARNING,
        parameters: compileParameters({
