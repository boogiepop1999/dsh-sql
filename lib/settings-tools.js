...
import { CONFIG_WRITE_WARNING, compileParameters, optionalString, requiredString, asRecord, textOutput, } from './tool-kit.js';
...
export function buildSettingsTools() {
    ...
    const sqlConfigSet = {
        name: 'sql_config_set',
        description: '改全局设置：maxRows / queryTimeoutMs / execTimeoutMs，至少给一个。\n\n' + CONFIG_WRITE_WARNING,
        parameters: compileParameters({
        ...
        async execute(rawArgs) {
            ...
            const changed = keys.filter((key) => args[key] !== undefined).map((key) => key + '=' + String(settings[key]));
    ...
    const sqlConnectionSet = {
        ...
        description: '新增或覆盖一个连接（**完全覆盖，不做隐式合并** —— 没给的字段就按默认值走），先 sql_settings 看现值。\n' +
        ...
        async execute(rawArgs) {
            ...
            const entry = { name, engine };
            if (args.readOnly !== undefined)
                entry.readOnly = args.readOnly === true;
            if (optionalString(args, 'description') !== undefined)
                entry.description = optionalString(args, 'description');
            if (engine === 'sqlite') {
                if (optionalString(args, 'file') !== undefined)
                    entry.file = optionalString(args, 'file');
            }
            else {
                if (optionalString(args, 'host') !== undefined)
                    entry.host = optionalString(args, 'host');
                if (args.port !== undefined)
                    entry.port = Number(args.port);
                if (optionalString(args, 'user') !== undefined)
                    entry.user = optionalString(args, 'user');
                if (args.password !== undefined)
                    entry.password = String(args.password);
                if (optionalString(args, 'database') !== undefined)
                    entry.database = optionalString(args, 'database');
            }
            mutate((draft) => {
    ...
    const sqlConnectionRemove = {
        ...
        description: '删除一个连接（连带关闭它的连接池），先 sql_settings 看现值。\n\n' + CONFIG_WRITE_WARNING,
        parameters: compileParameters({
            name: { type: 'string', required: true, description: '连接名。' },
        }),
        output: textOutput,
        async execute(rawArgs) {
            ...
            if (!exists) {
                const known = list.map((item) => item?.name).filter((item) => typeof item === 'string');
                throw new Error('连接 "' + name + '" 不存在，可用：' + (known.length > 0 ? known.join('、') : '（空，用 sql_connection_set 添加）'));
            }
            mutate((draft) => {
                draft.connections = (draft.connections ?? []).filter((item) => !(typeof item?.name === 'string' && item.name.toLowerCase() === name.toLowerCase()));
                return draft;
            });
