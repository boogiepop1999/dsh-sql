 ...
 */
export function isReadOnly(connection) {
 ...
 */
export function invalidReadOnly(connection) {
 ...
 */
export function requireMaxRows(settings) {
    const raw = settings.maxRows;
    if (raw === undefined)
        return 1000;
    if (typeof raw !== 'number' || !Number.isInteger(raw) || raw < 1 || raw > 10000) {
        throw new Error('maxRows 必须是 1~10000 之间的整数，收到 ' + JSON.stringify(raw) +
 ...
 */
export function connectionFieldKeys(engine) {
 ...
 */
export function fillConnectionKeys(connection) {
 ...
 * 不指定默认库时可用全限定名查询。
 */
export function missingConnectionFields(connection) {
    const missing = [];
...
export function splitConnectionsByEnv(settings) {
    ...
    for (const [name, connection] of Object.entries(settings.connections ?? {})) {
        ...
        else if (env === activeEnv)
            available[name] = connection;
        else
            excluded[name] = connection;
    }
